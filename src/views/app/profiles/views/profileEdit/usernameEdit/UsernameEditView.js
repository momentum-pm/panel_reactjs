            ...
            <ButtonView
              ...
              onClick={() =>
                navigator.clipboard.writeText(
                  `${Settings.CLIENT_URL}/@${this.getData().username}`
                )
              }
              type={BUTTON_TYPE.BUTTON}
            />
            <MasterColumn className={"en-box margin-one"}>
              <p className={"text"}>{`Momentum-AI.org/@${
                this.getData().username
              }`}</p>
            </MasterColumn>
          </Row>
          {this.getData().username_set ? null : (
            <p className={"padding-one-before-after center subtitle"}>
              {Res.string.no_username}
            </p>
          )}
          {this.getState().open ? (
              ...
              {this.getData().candidates.length > 0 ? (
                <div>
                  <h3 className={"center title"}>
                    {Res.string.username_candidates}
                  </h3>
