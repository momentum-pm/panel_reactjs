...
class ExerciseView extends RemoteStoreView {
  static getRemoteStore(props) {
    let episode = props.episode;
    let exerciseId = props.exercise.id;
    ...
    return (
      <div className="">
        <Row className={`padding-two centered`}>
          <h6 className={"header-style"}>
            سوال {normalizeNumber(this.getState().index + 1)}
          </h6>
          <MasterColumn>
        ...
        <div className="padding-two">
          ...
          {this.getState().answered ? (
            this.getState().isCorrect ? (
              <div className={`padding-one success row centered`}>
                <LatexView
                  latex={this.getState().answer}
                  className={"success"}
                />
                <p className="success">پاسخ صحیح است!</p>
              </div>
            ) : (
              <div className={`padding-one  row centered`}>
                <p className="danger">اشتباه است!</p>
              </div>
            )
          ) : null}
        </div>
...
export default connect(ExerciseView);
